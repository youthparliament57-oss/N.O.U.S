...
class TfIdfHashEmbedder(
    ...
    override suspend fun embed(text: String): FloatArray {
        ...
        for (tok in tokens) {
            val hash = sha256Bucket(tok, dimension)
    ...
    private fun sha256Bucket(token: String, dim: Int): Int {
    ...
    private companion object {
        // Pre-compile non-word character regex to avoid re-instantiating Regex on every embed() call.
        // Impact: Eliminates repeated Pattern/Regex allocations per memory embedding operation,
        // reducing GC overhead during bulk vector indexing / TF-IDF memory fallback searches.
