 ...
 *
 *  - All Regular Expressions (including the 38+ medicine matching regexes) are pre-compiled
 *    in static/companion memory. This prevents creating 50+ Regex objects per scanned document,
 *    substantially reducing CPU parsing overhead and JVM heap allocations during vision OCR operations.
 *
...
class PrescriptionScanner {
    ...
    internal fun extractClinicAddress(text: String): String? {
        ...
        val prefixMatch = CLINIC_ADDRESS_PREFIX_REGEX.find(text)
        prefixMatch?.groupValues?.getOrNull(1)?.trim()?.let { return it }
    ...
    internal fun extractPatientAge(text: String): String? {
        val match = PATIENT_AGE_REGEX_1.find(text) ?: PATIENT_AGE_REGEX_2.find(text)
        return match?.groupValues?.getOrNull(1)?.trim()
    }
    ...
    internal fun extractMedicines(text: String, blocks: List<OcrBlock>): List<PrescriptionMedicine> {
        ...
        // Find all medicine name occurrences using precompiled patterns.
    ...
    internal fun extractDuration(context: String): String? {
        val match = DURATION_REGEX_1.find(context) ?: DURATION_REGEX_2.find(context)
        return match?.value?.trim()
    }
    ...
    private companion object {
        ...
        // Precompiled regexes for medical prescription parsing
        ...
        private val CLINIC_ADDRESS_PREFIX_REGEX = Regex(
            "Address[:\\s]+(.+?)(?:\\n|$)",
            setOf(RegexOption.IGNORE_CASE, RegexOption.MULTILINE),
        )
        ...
        private val MEDICINE_REGEX_MAP: Map<String, Regex> = COMMON_MEDICINES_FOR_PARSING.associateWith { medicineName ->
