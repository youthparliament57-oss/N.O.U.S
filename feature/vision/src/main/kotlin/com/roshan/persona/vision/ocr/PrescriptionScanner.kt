...
class PrescriptionScanner {
    ...
    internal fun extractPatientAge(text: String): String? {
        val match = PATIENT_AGE_REGEX.find(text)
            ?: PATIENT_AGE_ALT_REGEX.find(text)
        return match?.groupValues?.getOrNull(1)?.trim()
    }
    ...
    internal fun extractMedicines(text: String, blocks: List<OcrBlock>): List<PrescriptionMedicine> {
        ...
        for ((medicineName, regex) in COMMON_MEDICINES_REGEX_MAP) {
    ...
    internal fun extractFollowUpDate(text: String): String? {
        // "Review after X days"
        val reviewMatch = REVIEW_AFTER_REGEX.find(text)
    ...
    private companion object {
        ...
        // Pre-compiled regex patterns for prescription fields.
        // Performance optimization: avoiding re-compilation of regular expressions during text processing.
        ...
        private val PATIENT_AGE_REGEX = Regex("\\bAge[:\\s]+(\\d{1,3})\\s*(?:years|yrs|y)?\\b", RegexOption.IGNORE_CASE)
        private val PATIENT_AGE_ALT_REGEX = Regex("\\b(\\d{1,3})\\s*[/\\-]\\s*[MFmf]\\b")
        ...
        private val REVIEW_AFTER_REGEX = Regex("Review\\s+(?:after|in)\\s+(\\d+)\\s*(?:days?|weeks?)", RegexOption.IGNORE_CASE)
        ...
        // Pre-compiled regex map for common medicines to avoid repeated Regex creation per medicine lookup.
        private val COMMON_MEDICINES_REGEX_MAP: List<Pair<String, Regex>> = COMMON_MEDICINES_FOR_PARSING.map { name ->
