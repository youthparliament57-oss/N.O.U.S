...
class PrescriptionScanner {
    ...
    internal fun extractDoctorQualification(text: String): String? {
        val match = QUALIFICATION_REGEX.find(text)
        return match?.value
    }
    ...
    internal fun extractClinicName(text: String): String? {
        ...
        val prefixMatch = CLINIC_NAME_PREFIX_REGEX.find(text)
    ...
    internal fun extractPatientAge(text: String): String? {
        val match = PATIENT_AGE_REGEX_1.find(text)
            ?: PATIENT_AGE_REGEX_2.find(text)
        return match?.groupValues?.getOrNull(1)?.trim()
    }
    ...
    internal fun extractPatientGender(text: String): String? {
        val match = PATIENT_GENDER_REGEX.find(text)
    ...
    internal fun extractPrescriptionDate(fields: List<OcrField>, text: String): String? {
        ...
        val match = PRESCRIPTION_DATE_REGEX.find(text)
        return match?.groupValues?.getOrNull(1)?.trim()
    }
    ...
    internal fun extractMedicines(text: String, blocks: List<OcrBlock>): List<PrescriptionMedicine> {
        ...
        // Find all medicine name occurrences using pre-compiled regexes.
        for ((medicineName, regex) in MEDICINE_REGEX_MAP) {
    ...
    internal fun extractFrequency(context: String): String? {
        ...
        val abbrevMatch = FREQUENCY_ABBREV_REGEX.find(context)
        if (abbrevMatch != null) return abbrevMatch.value

        // Numeric pattern (1-0-1, 1-1-1, 0-0-1).
        val numericMatch = FREQUENCY_NUMERIC_REGEX.find(context)
        if (numericMatch != null) return numericMatch.value

        // English phrases.
        val phraseMatch = FREQUENCY_PHRASE_REGEX.find(context)
    ...
    internal fun extractDuration(context: String): String? {
        val match = DURATION_REGEX_1.find(context)
            ?: DURATION_REGEX_2.find(context)
        return match?.value?.trim()
    }
    ...
    internal fun extractDiagnosis(text: String): String? {
        val match = DIAGNOSIS_REGEX.find(text)
        return match?.groupValues?.getOrNull(1)?.trim()
    }
    ...
    internal fun extractFollowUpDate(text: String): String? {
        // "Review after X days"
        val reviewMatch = REVIEW_FOLLOW_UP_REGEX.find(text)
        if (reviewMatch != null) return reviewMatch.value.trim()

        // "Follow-up: DATE"
        val followUpMatch = DATE_FOLLOW_UP_REGEX.find(text)
    ...
    private companion object {
        ...
        // Pre-compiled regex instances to eliminate regex compilation overhead during OCR scanning
        private val DOCTOR_NAME_REGEX = Regex("Dr\\.?\\s+([A-Z][a-z]+(?:\\s+[A-Z][a-z]+){0,3})")
        private val QUALIFICATION_REGEX = Regex("\\b(MBBS|MD|MS|DM|MCh|BAMS|BHMS|BDS)\\b")
        private val CLINIC_NAME_PREFIX_REGEX = Regex(
        ...
        private val PATIENT_AGE_REGEX_1 = Regex("\\bAge[:\\s]+(\\d{1,3})\\s*(?:years|yrs|y)?\\b", RegexOption.IGNORE_CASE)
        private val PATIENT_AGE_REGEX_2 = Regex("\\b(\\d{1,3})\\s*[/\\-]\\s*[MFmf]\\b")
        private val PATIENT_GENDER_REGEX = Regex("\\b(Male|Female|M|F)\\b")
        private val PRESCRIPTION_DATE_REGEX = Regex("\\bDate[:\\s]+(\\d{1,2}[/\\-]\\d{1,2}[/\\-]\\d{2,4})\\b", RegexOption.IGNORE_CASE)
        private val DOSAGE_REGEX = Regex("\\b(\\d{1,4})\\s*(mg|ml|mcg|tablets?|capsules?|drops?|teaspoons?)\\b", RegexOption.IGNORE_CASE)
        private val FREQUENCY_ABBREV_REGEX = Regex("\\b(OD|BD|TDS|TID|QID|HS|SOS|QD|BID)\\b")
        private val FREQUENCY_NUMERIC_REGEX = Regex("\\b([0-2]-[0-2]-[0-2])\\b")
        private val FREQUENCY_PHRASE_REGEX = Regex("\\b(once daily|twice daily|thrice daily|every\\s+\\d+\\s+hours?|as needed|at bedtime)\\b", RegexOption.IGNORE_CASE)
        private val DURATION_REGEX_1 = Regex("\\b(?:for\\s+)?(\\d+)\\s*(days?|weeks?|months?)\\b", RegexOption.IGNORE_CASE)
        private val DURATION_REGEX_2 = Regex("\\bx\\s*(\\d+)\\s*(days?|weeks?)\\b", RegexOption.IGNORE_CASE)
        ...
        private val DIAGNOSIS_REGEX = Regex(
            "(?:Diagnosis|Advice|Provisional Diagnosis)[:\\s]+(.+?)(?:\\n|$)",
            setOf(RegexOption.IGNORE_CASE, RegexOption.MULTILINE),
        )
        private val REVIEW_FOLLOW_UP_REGEX = Regex("Review\\s+(?:after|in)\\s+(\\d+)\\s*(?:days?|weeks?)", RegexOption.IGNORE_CASE)
        private val DATE_FOLLOW_UP_REGEX = Regex("Follow[-\\s]?up[:\\s]+(\\d{1,2}[/\\-]\\d{1,2}[/\\-]\\d{2,4})")
        ...
        private val MEDICINE_REGEX_MAP: List<Pair<String, Regex>> = COMMON_MEDICINES_FOR_PARSING.map { name ->
            name to Regex("\\b${Regex.escape(name)}\\b", RegexOption.IGNORE_CASE)
        }
    }
}
