...
class PrescriptionScanner {
    ...
    internal fun extractDoctorName(fields: List<OcrField>, text: String): String? {
        ...
        // Fallback: pre-compiled regex on text.
    ...
    internal fun extractDoctorQualification(text: String): String? {
        val match = DOCTOR_QUAL_REGEX.find(text)
        return match?.value
    }
     ...
     *
     * Algorithm:
     *  1. Find all known medicine names (lookup against pre-compiled COMMON_MEDICINES_REGEX_LIST).
     *  2. For each medicine, search the next 100 characters for:
    ...
    internal fun extractMedicines(text: String, blocks: List<OcrBlock>): List<PrescriptionMedicine> {
        ...
        // Performance Optimization: Use pre-compiled Regex instances to avoid re-compiling ~35 regex patterns per prescription parsing pass.
        for ((medicineName, regex) in COMMON_MEDICINES_REGEX_LIST) {
    ...
    internal fun extractFollowUpDate(text: String): String? {
        // "Review after X days"
        val reviewMatch = REVIEW_MATCH_REGEX.find(text)
        if (reviewMatch != null) return reviewMatch.value.trim()

        // "Follow-up: DATE"
        val followUpMatch = FOLLOW_UP_DATE_REGEX.find(text)
    ...
    private companion object {
        ...
        // Performance Optimization: Pre-compile Regex patterns in companion object to avoid repeated compilation and GC overhead during parsing.
        private val DOCTOR_NAME_REGEX = Regex("Dr\\.?\\s+([A-Z][a-z]+(?:\\s+[A-Z][a-z]+){0,3})")
        private val DOCTOR_QUAL_REGEX = Regex("\\b(MBBS|MD|MS|DM|MCh|BAMS|BHMS|BDS)\\b")
        ...
        private val REVIEW_MATCH_REGEX = Regex("Review\\s+(?:after|in)\\s+(\\d+)\\s*(?:days?|weeks?)", RegexOption.IGNORE_CASE)
        private val FOLLOW_UP_DATE_REGEX = Regex("Follow[-\\s]?up[:\\s]+(\\d{1,2}[/\\-]\\d{1,2}[/\\-]\\d{2,4})", RegexOption.IGNORE_CASE)
        ...
        // Performance Optimization: Pre-compile medicine search Regexes to avoid creating ~35 Regex instances per prescription parsing pass.
        private val COMMON_MEDICINES_REGEX_LIST = COMMON_MEDICINES_FOR_PARSING.map { name ->
